...
figure(41) ; set(gcf, 'name', 'Part 3.1: Data') ; clf ;

subplot(5,2,1) ; imagesc(imdb.images.data(:,:,:,1)) ;
axis off image ; title('Input (blurred)') ;

subplot(5,2,2) ; imagesc(imdb.images.label(:,:,:,1)) ;
axis off image ; title('Desired output (sharp)') ;

subplot(5,2,3) ; imagesc(imdb.images.data(:,:,:,2)) ;
axis off image ; title('Input (blurred)') ;

subplot(5,2,4) ; imagesc(imdb.images.label(:,:,:,2)) ;
axis off image ; title('Desired output (sharp)') ;

subplot(5,2,5) ; imagesc(imdb.images.data(:,:,:,3)) ;
axis off image ; title('Input (blurred)') ;

subplot(5,2,6) ; imagesc(imdb.images.label(:,:,:,3)) ;
axis off image ; title('Desired output (sharp)') ;

subplot(5,2,7) ; imagesc(imdb.images.data(:,:,:,4)) ;
axis off image ; title('Input (blurred)') ;

subplot(5,2,8) ; imagesc(imdb.images.label(:,:,:,4)) ;
axis off image ; title('Desired output (sharp)') ;

subplot(5,2,9) ; imagesc(imdb.images.data(:,:,:,5)) ;
axis off image ; title('Input (blurred)') ;

subplot(5,2,10) ; imagesc(imdb.images.label(:,:,:,5)) ;
axis off image ; title('Desired output (sharp)') ;
...
figure(43) ; set(gcf, 'name', 'Part 3.4: Results on the training set') ;
showDeblurringResult(net, imdb, train(1:30:151)) ;

figure(44) ; set(gcf, 'name', 'Part 3.4: Results on the validation set') ;

...
%Load VGG face model
net = load('data/vgg-face.mat') ;
% net = load('data/imagenet-vgg-verydeep-16.mat')

net.layers(36) = []; %36번째 class score layer제거 
net.layers(end) = []; %37번째 softmax layer 제거

f = 1/100;
net.layers{32} = struct('type', 'conv', ...
                           'filters', f*randn(2,2,512,4096, 'single'), ...          
                           'biases', zeros(1, 4096, 'single'), ...
                           'stride', 1, ...
                           'pad', 0,'learningRate',[1 2],'weightDecay', [1 0]) ;

net.layers{36} = struct('type', 'conv', ...
                           'filters', f*randn(1,1,4096,19, 'single'), ...
                           'biases', zeros(1, 19, 'single'), ...
                           'stride', 1, ...
                           'pad', 0,'learningRate',[1 2],'weightDecay', [1 0]) ;
...
for i=1:numel(net.layers)
    ...
    if ~isfield(net.layers{i}, 'filters')
        net.layers{i}.filters = net.layers{i}.weights{1}; %filters 값을 pretrained vgg-face weight값으로 초기화
    end
    
    if ~isfield(net.layers{i}, 'biases')
        net.layers{i}.biases = net.layers{i}.weights{2}; %biases 값을 pretrained vgg-face weight값으로 초기화
    end
    ...
    if ~isfield(net.layers{i}, 'filtersLearningRate')
        net.layers{i}.filtersLearningRate = net.layers{i}.learningRate(1);
    end
    if ~isfield(net.layers{i}, 'biasesLearningRate')
        net.layers{i}.biasesLearningRate = net.layers{i}.learningRate(2);
    end
    if ~isfield(net.layers{i}, 'filtersWeightDecay')
        net.layers{i}.filtersWeightDecay = net.layers{i}.weightDecay(1);
    end
    if ~isfield(net.layers{i}, 'biasesWeightDecay')
        net.layers{i}.biasesWeightDecay = net.layers{i}.weightDecay(2);
    end
end

load('mat\CMU_DB(64x64x3)_train_validation.mat') ; 

for i=1:1:length(imdb.images.label)
    imdb.images.data_resized(:,:,:,i) = imresize(imdb.images.data(:,:,:,i), [64 64]); 
end
imdb.images.data = imdb.images.data_resized; % resized된 영상들 data에 저장
imdb.images.data_resized = []; % resized된 data 제거



net.meta.normalization.averageImage(:,:,1) = (net.meta.normalization.averageImage(:,:,1)+imdb.info.average(1))/2;
net.meta.normalization.averageImage(:,:,2) = (net.meta.normalization.averageImage(:,:,2)+imdb.info.average(2))/2;
net.meta.normalization.averageImage(:,:,3) = (net.meta.normalization.averageImage(:,:,3)+imdb.info.average(3))/2;

for i=1:1:length(imdb.images.label)
    imdb.images.data(:,:,:,i) = bsxfun(@minus, imdb.images.data(:,:,:,i),net.meta.normalization.averageImage) ;
end


% -------------------------------------------------------------------------
% Part 4: train and evaluate the CNN
% -------------------------------------------------------------------------
%================= 이 부분 수정 =====================
opts.train = [] ;
opts.val = [] ;
...
opts.momentum = 0.9 ;
opts.errorType = 'multiclass' ; %binary 분류일 경우에 이부분 수정
...
for epoch=1:opts.numEpochs
  ...
  for t=1:opts.batchSize:numel(train)
    ...
    [im, labels] = getBatch_vggFace(imdb, batch) ; % vgg_face model에 적합하게 getBatch.m함수 수정
    if opts.prefetch
...
%Test data load
load('network\net-epoch-25.mat') ;
net.layers(end) = []; %softmax log-loss 제거 / testing을 위해
load('mat\CMU_DB(64x64x3)_test.mat') ; %imdb 변수 생성

for i=1:1:length(imdb.images.label)
    imdb.images.data_resized(:,:,:,i) = imresize(imdb.images.data(:,:,:,i), [64 64]); % 224 x 224 x 3으로 resizing
end
imdb.images.data = imdb.images.data_resized; % resized된 영상들 data에 저장
imdb.images.data_resized = []; % resized된 data 제거
...
for i=1:1:length(imdb.images.label)
    ...
    scores{i} = squeeze(gather(res(end).x)) ;
    [bestScore(i), best(i)] = max(scores{i}) ;
end

sum = 0;
for i=1:1:length(imdb.images.label)
    if(imdb.images.label(i) == best(i))
        sum = sum+1;
    end
        
end
 'overall accuracy'
 (sum/length(imdb.images.label))
